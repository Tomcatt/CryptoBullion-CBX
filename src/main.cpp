...
const string strMessageMagic = "CryptoBullion Signed Message:\n";
...
unsigned int GetStakeTargetSpacing(unsigned int nTime){
    if(nTime >= (unsigned int) HARDFORK_TIME)
        ...
        return OLD_nStakeTargetSpacing;
}
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout) {
        ...
        if (whichType == TX_NULL_DATA)
            nDataOut++;
        else if (txout.IsDust(MIN_TX_FEE)) {
            // dust
            return false;
        }
    }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, unsigned int nHeight, int64 nMoneySupply)
{
    ...
    if(fTestNet || nTime > PROTOCOL_SWITCH_TIME)
    {
        ...
        // CryptoBullion: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
static const int64 nTargetSpacingWorkMax = 12 * OLD_nStakeTargetSpacing; // 12 minutes
...
unsigned int static GetNextTargetRequiredHybrid(const CBlockIndex* pindexLast, bool fProofOfStake){
    CBigNum bnTargetLimit = bnProofOfWorkLimit;
    ...
    int64 nTargetSpacing = fProofOfStake ? GetStakeTargetSpacing(pindexLast->nTime) : min(nTargetSpacingWorkMax, (int64) GetStakeTargetSpacing(pindexLast->nTime) * (1 + pindexLast->nHeight - pindexPrev->nHeight));
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal cryptobullion miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Also skip if signature checking is diabled due to TX DB upgrade/migration.
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CryptoBullion
    bool fStrictPayToScriptHash = true; // Always active in CryptoBullion
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // cgb: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // cgb: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // cgb: record proof-of-stake hash value
    ...
    // cgb: compute stake modifier
    uint64 nStakeModifier = 0;
    bool fGeneratedStakeModifier = false;
    if (!ComputeNextStakeModifier(pindexNew->pprev, nStakeModifier, fGeneratedStakeModifier, pindexNew->nTime))
    ...
    if (!txdb.TxnBegin())
        return false;
    CDiskBlockIndex bi = CDiskBlockIndex(pindexNew);
    txdb.WriteBlockIndex(bi);
    if (!txdb.TxnCommit())
        return false;
...
bool CBlock::AcceptBlock()
{
    ...
    // cgb: check that the block satisfies synchronized checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // cgb: verify hash target and signature of coinstake tx
    ...
    if (!pblock->IsProofOfStake() && pcheckpoint && pblock->hashPrevBlock != hashBestChain && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
    ...
    // cgb: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CryptoBullion", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
//vMerkleTree: ea6fed5e2
       ...
       if (false  && (block.GetHash() != hashGenesisBlock))
       {
        ...
        // cgb: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // cgb: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
bool LoadExternalBlockFile(FILE* fileIn, ExternalBlockFileProgress *progress, int blockfileVersion)
{
    ...
    int nLoaded = 0;
    {
        ...
        try {
            ...
            char xx[2555];
            sprintf(xx, "%s() : Deserialize or I/O error caught during load\n",
                   __PRETTY_FUNCTION__);
...
string GetWarnings(string strFor)
{
    ...
    /* remove warning until automatic sync is working correctly
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptobullionMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("CryptobullionMiner : ProcessBlock, block not accepted");
    }
